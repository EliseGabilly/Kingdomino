...
public class PageFinDynastie {
	...
	protected void createContents(Shell shell) {
		...
		// un tableau contenant les scores de chaque joueur est cr�e pour chaque manche du mode dynastie
		...
		for (int numJoueur=0; numJoueur<size; numJoueur++) {
			...
			//si le score du joueur est le score maximum de la manche concern�e on l'affiche en gras
		...
		btnQuitter.addListener(SWT.Selection, new Listener() {
			//si le bouton quitter est cliqu� on coupe le son et on ferme la fenetre
			public void handleEvent(Event e) {
				switch (e.type) {
				case SWT.Selection:
					try {
	...
	/**
	 * La fonction max renvoie le maximum du tableau d'entier entr� en param�tre
	 * Elle n'est utilis�e que dans la classe PageFinDynastie
	 * @param tab

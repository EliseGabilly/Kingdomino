...
public class PageDeFin {
	...
	public void createContents(Shell shell) {
		...
		//Si l'utilisateur change la taille de la fen�tre la fonction refresh est appel�e pour actualiser la taille des �l�ments
		...
		if (monJeu.getDynastie() == 0) {
			btnRestart.addListener(SWT.Selection, new Listener() {
				public void handleEvent(Event e) {
					...
					case SWT.Selection:
						disposeContent(); 
						for (Joueur j : listeJoueurLoc) { //On cr�e un nouveau royaume vide pour chaque joueur
						...
						//On relance une partie vierge avec les m�me param�tres
			...
			} else { // cas ou on est dans une manche interm�diaire du mode dynastie le bouton restart est remplac� par un bouton continuer
		...
		//si le bouton menu est cliqu� on ferme la fenetre actuelle et on cr�e puis affiche la page d'acceuil
		...
		//on initialise les meilleures carac�ristiques des royaumes
		...
		for (int i = 0; i < listeJoueurLoc.size(); i++) {
			...
			//les if suivants sont caract�riqtiques d'une fonction de recherche de maximum
		...
		for (int numJoueur = 0; numJoueur < listeJoueurLoc.size(); numJoueur++) {
			...
			if (res[0] == scoreMax) {// le meilleur score est affich� en gras
	...
	/**
	 * La fonction refresh actualise la taille des �l�ments en fonction de la taille de la fen�tre
	 */
	public void refresh() {

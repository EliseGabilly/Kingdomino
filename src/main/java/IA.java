...
public class IA {
	 ...
	 * @param royaume
	 * @param tuile
	 * @return la liste de toute les coordon�es ou il est possible de place la
	 *         premiere case
	 */
	...
	public List<List<List<Double>>> pointsParPlace(Royaume royaume, List<List<int[]>> placePossible, String[][] tuile) {
		...
		String[] del = new String[2]; // utilis� pour suprimer les tuille apres avoir compt� les points
		Double b;
		...
		for (int nbCase = 0; nbCase < 2; nbCase++) {
			...
			for (int[] unePlace : placePossible.get(nbCase)) {
				...
				// l'impossibilit� de jouer dans cette direction)
				for (int[] d : direction) {
					try {
	 ...
	 *         a d'autre (agrandissement de terrain, sans trou, centr�...)
	 */
	public Double pointBonus(Royaume royaume, int x1, int y1, int x2, int y2) {
		...
		for (int[] d : direction) {
			try {
				// Deux cases vides du meme cot� ne cimpte pas comme un trou
		...
		// Garder un royaume centr� (si c'est encore possible)
		...
		// Eviter de posser a cot� du chateau
		for (int[] d : direction) {
			try {
	 ...
	 * qui rapportent le maximum de point puis choisit all�atoirement l'une de ces
	 * places
	 * 
			...
			if (pointsParPlace.get(nbCase).size() != 0) {
				for (int i = 0; i < pointsParPlace.get(nbCase).size(); i++) {
					for (d = 0; d < 4; d++) {
						...
						if (temp > max) { // si on trouve une valeur plus grande que l'ancien max on r�initialise la
											// liste
	 ...
	 * @param royaume
	 * @param tempNextTuile
	 * @return le numero de la tuile selectionn�e
	 */
	...
	public Double maxDePoint(List<List<List<Double>>> pointPossible, List<List<int[]>> placePossible) {
		Double max = -1.0;
		Double temp;
		// pour chaque case on verifi qu'on ne d�passe pas le maximum enrengistr�
